...
export interface ProgressiveFluxLoaderProps {
  ...
  showLabel?: boolean;
  /** `compact` fits inside dialogs; `default` is full hero sizing. */
...
function FluxLabel({ label, reduced, compact, className }: FluxLabelProps) {
  const base = cn(
    "absolute inset-0 flex items-center justify-center text-center font-semibold tracking-tight text-muted-foreground",
    compact ? "text-sm sm:text-base" : "text-3xl sm:text-4xl",
    className,
  );
...
export function ProgressiveFluxLoader({
  ...
  return (
    ...
    >
      {showLabel && (
        <div
          className={cn(
            ...
            compact ? "h-9 sm:h-10" : "h-16",
          )}
          style={reduced ? undefined : { perspective: compact ? "720px" : "1000px" }}
        >
          <FluxLabel
            label={label}
            reduced={reduced}
      ...
      <div
        className={cn(
          "relative w-full overflow-hidden rounded-full bg-muted shadow-[inset_0_2px_3px_rgba(0,0,0,0.09),inset_0_-1px_2px_rgba(255,255,255,0.7)] dark:shadow-[inset_0_2px_3px_rgba(0,0,0,0.45),inset_0_-1px_2px_rgba(255,255,255,0.05)]",
          compact ? "h-3" : "h-5",
          barClassName,
        )}
        role="progressbar"
